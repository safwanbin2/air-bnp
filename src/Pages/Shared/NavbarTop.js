    ...
    return (
        <>
            <div ref={menuRef} className={`${isOpen ? "h-40 absolute top-0 left-0 w-full bg-white shadow" : "h-20"} flex justify-center items-center border-b z-10 transition-all duration-200`}>
                ...
                <div onClick={() => setIsOpen(!isOpen)} className={`w-11/12 md:w-[28%] grid-cols-3 px-4 py-2 rounded-3xl border shadow-md hover:shadow-lg font-semibold items-center gap-1 cursor-pointer  ${isOpen ? "hidden" : "grid"}`}>
                ...
                <div className={`${isOpen ? "" : "hidden"} pb-4 `}>
                    ...
                    <div className='h-20 py-2'>
                        <div className='h-full w-full md:min-w-[620px] rounded-full bg-base-200 flex justify-center items-center shadow-md relative'>
            ...
            <button onClick={handleRefresh} className='hidden md:block absolute top-6 left-14 z-20 text-primary text-2xl font-semibold'>airbnp</button>
            <button className='hidden md:block absolute top-7 right-14 z-20 '>
